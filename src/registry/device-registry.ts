...
import {
  allowedTransitions,
  type DeviceStateChangeSource,
  type DeviceStateTransition,
} from '../domain/device-state-transition.js';
...
export class DeviceRegistry {
  ...
  findAvailable(criteria: DeviceCriteria): Device[] {
    return this.find(criteria).filter((device) => device.status === 'available');
  }

  find(criteria: DeviceCriteria): Device[] {
    return this.list().filter((device) => {
      if (criteria.platform && device.platform !== criteria.platform) return false;

      if (criteria.kind && device.kind !== criteria.kind) return false;

      if (criteria.performanceClass && device.performanceClass !== criteria.performanceClass) return false;

      if (criteria.resolution && !device.capabilities.resolutions.includes(criteria.resolution)) return false;

      if (criteria.hdr && !device.capabilities.hdrFormats.includes(criteria.hdr)) return false;

      if (criteria.codec && !device.capabilities.codecs.includes(criteria.codec)) return false;

      if (criteria.games !== undefined && device.capabilities.games !== criteria.games) return false;

      if (criteria.minimumMemoryMb !== undefined && (device.capabilities.memoryMb ?? 0) < criteria.minimumMemoryMb)
        return false;

      if (
        criteria.requiredOperations &&
        !criteria.requiredOperations.every((operation) => device.supportedOperations.includes(operation))
      )
        return false;
